...
async def test_async_get_timers(mock_aio_protocol, caplog: pytest.LogCaptureFixture):
    ...
    assert len(timers) == 6
    assert (
        timers[0].toBytes() == b"\xf0\x16\x01\x04\x00+\x00\x00\x00\x00\x00\x00\x00\x0f"
    )
    assert str(timers[0]) == "[OFF] 00:43  Once: 2022-01-04  Color: (25, 71, 255)"
    assert (
        timers[1].toBytes() == b"\xf0\x16\x01\x04\x04,\x00\x00\x00\x00\x00\x00\x00\x0f"
    )
    assert str(timers[1]) == "[OFF] 04:44  Once: 2022-01-04  Color: chartreuse"
    assert (
        timers[2].toBytes()
        == b"\xf0\x16\x01\x03\x16\x1f\x00\x00\x00\x00\x00\x00\x00\x0f"
    )
    assert str(timers[2]) == "[OFF] 22:31  Once: 2022-01-03  Color: red"
    assert (
        timers[3].toBytes()
        == b"\xf0\x16\x01\x03\x17\x13\x00\x00\x00\x00\x00\x00\x00\x0f"
    )
    assert str(timers[3]) == "[OFF] 23:19  Once: 2022-01-03  Color: (129, 129, 129)"
    assert (
        timers[4].toBytes() == b"\xf0\x16\x01\x03\x17(\x00\x00\x00\x00\x00\x00\x00\x0f"
    )
    assert str(timers[4]) == "[OFF] 23:40  Once: 2022-01-03  Color: lime"
    assert (
        timers[5].toBytes() == b"\xf0\x16\x01\x04\x07,\x00\x00\x00\x00\x00\x00\x00\x0f"
    )
    assert str(timers[5]) == "[OFF] 07:44  Once: 2022-01-04  Color: (33, 0, 255)"
